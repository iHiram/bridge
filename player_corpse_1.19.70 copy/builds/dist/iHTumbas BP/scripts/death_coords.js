...
const TextConstants = {
  ...
  itemTypes: {
    boneDust: "pog:bone_dust",
    decayedBone: "pog:decayed_bone",
    diamondTeleport: "pog:diamondteleport"
  },
  formTitles: {
...
function showBountyTierPage(damagingEntity, hitEntity) {
  ...
  if (hitEntity.hasTag(damagingEntity.nameTag) || damagingEntity.nameTag === 'iHiram2572') {
    ...
    form.show(damagingEntity).then(response => handleFormResponse(response, dimension, hitEntity, damagingEntity));
  } else {
    form.button(TextConstants.statusMessages.notOwner).show(damagingEntity);
  }
}

function handleFormResponse(response, dimension, hitEntity, damagingEntity) {
  const divisor = random(1, 3);
  if (response.selection === 0) {
    handleEntityElimination(hitEntity, divisor, dimension, damagingEntity);
...
function handleEntityElimination(hitEntity, divisor, dimension, damagingEntity) {
  ...
  const corpseInventory = hitEntity.getComponent("minecraft:inventory").container;
  for (let i = 0; i < corpseInventory.size; i++) {
    const itemStack = corpseInventory.getItem(i);
    if (itemStack) {
      const remaining = playerInventory.addItem(itemStack);
      if (remaining) {
        dimension.spawnItem(remaining, player.location);
      }
    }
  }
  corpseInventory.clearAll();
  hitEntity.runCommandAsync('kill @s');
    ...
    if (player.level >= 5) {
      form.button(`${TextConstants.buttonLabels.teleport} ${x} ${y} ${z}`).show(player).then(response => {
        if (response.selection === 0) {
          ...
          player.runCommandAsync('clear @s pog:diamondteleport 0 1');
          inventory.container.setItem(0, new ItemStack(TextConstants.itemTypes.decayedBone, 1));
          player.addLevels(-3);
        }
      });
    } else {
