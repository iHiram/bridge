...
const TAGS = {
    ...
    teleport: 'ef-teleport'
};

const EXPLOSION_OPTIONS = { allowUnderwater: true, breaksBlocks: false, causesFire: true };

// Calculamos las probabilidades una sola vez, con un máximo de 0.7
const difficultyLevel = 'Normal';
const maxProbability = 0.7;
...
const PROBABILITIES = {
    infinity: 10000000,
    tProb: 1000,
    mobsRegular: 0.6 * difficultyMultiplier,
    mobsSpider: 0.6 * difficultyMultiplier,
    mobsRare: 0.4 * difficultyMultiplier,
    mobsExceptions: 0.3 * difficultyMultiplier,
    soCommun: 0.7 * difficultyMultiplier,
    commun: 0.6 * difficultyMultiplier,
    rare: 0.4 * difficultyMultiplier,
    soRare: 0.25 * difficultyMultiplier,
    legent: 0.15 * difficultyMultiplier,
    soLegent: 0.03 * difficultyMultiplier
};
...
console.warn(difficultyLevel);
console.warn(PROBABILITIES);
...
world.afterEvents.entitySpawn.subscribe(evnt => {
    ...
    if (listMobs.includes(entity.typeId) && entity.nameTag === '' && !entity.hasTag(TAGS.hasEffect)) {
        ...
        // Añadimos la etiqueta de teletransporte con probabilidad "rare"
        if (irand <= PROBABILITIES.rare * PROBABILITIES.tProb) {
            entity.addTag(TAGS.teleport);
        }
...
world.afterEvents.entityHurt.subscribe(data => {
    ...
    if (hitEntity && entity) {
        if (listMobs.includes(entity.typeId)) {
            ...
            // Lógica de teletransporte basada en la etiqueta "teleport"
            if (entity.hasTag(TAGS.teleport)) {
                teleportToPlayer(hitEntity, entity);
            }
        }
    }
});
...
function addEffects(entity) {
    ...
    effectTags.forEach(({ effect, varAmplifier, prob, tag, condition }) => {
        if (condition === undefined || condition) {
            irand = random(1, PROBABILITIES.tProb);
            if (irand <= prob * PROBABILITIES.tProb) {
                if (effect) {
                    entity.addEffect(effect, PROBABILITIES.infinity, { amplifier: random(...varAmplifier), showParticles: true });
                    contEffect++;
                } else if (tag) {
                    entity.addTag(tag);
                    contEffect++;
...
function setNameTag(entity, name) {
    // Verifica si la entidad existe y tiene la propiedad nameTag
    if (entity && entity.nameTag !== undefined) {
        entity.nameTag = name;  // Establece el nombre directamente
    } else {
        console.log("Error: La entidad no tiene la propiedad nameTag o no es válida.");
    }
}
...
function teleportToPlayer(entity, source) {
    const playerLocation = source.location;
    const offsetX = random(-5, 5);
    const offsetZ = random(-5, 5);
    ...
    entity.teleport(teleportLocation, { dimension: source.dimension });
    ...
    world.playSound('minecraft:entity.enderman.teleport', soundOptions);

    // Show enderman particles
    const dimension = world.getDimension(entity.dimension.id);
    if (dimension) {
        dimension.spawnParticle("minecraft:enderman_teleport", teleportLocation, {
            x: 0,
            y: 0,
            z: 0
        });
...
function changeMob(entity) {
    if (entity.typeId === ENTITY_TYPES.spider) {
        ...
        irand = random(1, PROBABILITIES.tProb);
        if (irand <= PROBABILITIES.rare) {
            entity.kill();  // Mata al creeper original
            entity.runCommandAsync('kill @e[type=item,r=1]');
